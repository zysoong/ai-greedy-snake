...
#model.fit(input, output, epochs=10, batch_size = 100)
print('###########PREDICT##############')
print(model.predict(input)[3])
print('###########ARGMAX##############')
argmax = tf.math.argmax(model.predict(input)).numpy()[0]
print(model.predict(input)[argmax])
